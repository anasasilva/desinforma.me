...
/**
 * Adapted from https://github.com/3DJakob/react-tinder-card/
  ...
  React.useLayoutEffect(() => {
    ...
    element.current.addEventListener(('touchmove'), (ev) => {
      ...
      if (didCardDragStart) {
        ...
        const coordinates = touchCoordinatesFromEvent(ev);
        const speed = { x: coordinates.x + offset.x, y: coordinates.y + offset.y }
        const norm = Math.sqrt(speed.x * speed.x + speed.y * speed.y);
        if ((Math.abs(speed.y) / norm) > 0.6 || norm < 5) {
